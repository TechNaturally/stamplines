...
export default class LinePanel extends Panel {
  ...
  setData(data={}) {
    ...
    if (this.data && this.data.item && this.data.item.data) {
      ...
      if (itemData.Line) {
        this.setTitle(itemData.Line.name || itemData.Line.id);
      }
      ...
      if (this.DOM && this.DOM.labelPositionGroup) {
        itemData.labelPosition = (itemData.labelPosition || 'top');
  ...
  generateDOM() {
    ...
    // label input
    formGroup = $('<div class="form-group input-item-label"></div>');
    ...
    formInput = $('<div class="sl-position-input sl-position-2-way-vertical"></div>');
    ...
    formInput.append($('<span class="divider"></span>'));
